      ...
      $resume = $rscore->fetch_assoc();
		...
		?>
          ...
          <div class="card shadow mb-4">
            <div class="card-body">
              <form class="table-responsive" action="taken" method="post">
            ...
            <div class="card-body">
              <form class="table-responsive" action="taken" method="post">
                <table class="table table-bordered" id="RateTable" width="100%" cellspacing="0">
                  ...
                  <tbody>
                  	<?php
                  		$userq = $conex->query("SELECT U.FNAME, U.LNAME, U.RESUMESCORE, U.EMAIL, U.PHONE, U.QUALI, U.EXP, U.STATUS, U.CODE USER_CODE, R.QUEST_CODE, SUM(R.ANS) ANS FROM `users` U, result R WHERE U.CODE = R.USER_CODE AND R.ANS = 1 AND U.STATUS = 3 GROUP BY R.USER_CODE ORDER BY (SUM(R.ANS)+U.QUALI+u.EXP) ASC");
                  		while ($data = $userq->fetch_assoc()) {
                  	?>
                  		<tr>
                  			<td><input type="checkbox" id="<?=$data['USER_CODE'];?>" name="users[]" value="<?=$data['USER_CODE'];?>"></td>
                        ...
                        <td><?php echo $data['QUALI']+$data['EXP']+$data['ANS']+$data['RESUMESCORE']?></td>
                  			<td><a href="taken?view=<?=($data['USER_CODE']);?>">View</a></td>
                  		</tr>
                  	<?php
                  		}
                  	?>
